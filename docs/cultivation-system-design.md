...
| T1.1 ✅（v28，2026-09-10） | 模板定义 + 培养引擎（阶段推进→学习产出复用）+ 教育证据分级 | 跑完一个模板阶段，知识/技能/证据按分级落库 |
| T1.2 ✅（无新迁移，2026-09-10） | 际遇事件 + 人格成型 + 阶段评估接线 | 同模板不同 seed 产出分布有差异；评估全走证据聚合 |
| T1.3 ✅（无新迁移，2026-09-10） | 培养 UI（建角色→选模板/自由→时间线→成品档案） | 页面全流程可走通，i18n 中英 |
