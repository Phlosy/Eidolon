...
export function RuntimePage() {
  ...
  const unhealthy = runtimes.filter((runtime) =>
  ...
  return (
    <div className="space-y-6 panel-enter">
      <PageHeader
        icon={ServerCog}
        title={t("runtime:controlCenter.title")}
        description={t("runtime:controlCenter.description")}
      />
      <section className="command-panel relative overflow-hidden p-5 md:p-6">
        <div className="relative grid gap-5 md:grid-cols-[1.2fr_repeat(3,minmax(0,.6fr))]">
          <div>
            <p className="type-kicker text-primary">
              {t("runtime:controlCenter.liveInfrastructure")}
            </p>
          ...
          {[
            [Activity, t("runtime:controlCenter.activeNodes"), `${running}/${runtimes.length}`],
            [Gauge, t("runtime:controlCenter.incidents"), unhealthy],
            [Box, t("runtime:controlCenter.images"), imagesQuery.data?.length ?? 0],
        ...
        <div className="mb-3 flex items-end justify-between">
          <div>
            <p className="type-kicker text-muted-foreground">
              {t("runtime:controlCenter.nodesKicker")}
            </p>
            <h2 className="mt-1 text-lg font-semibold">{t("runtime:controlCenter.nodesTitle")}</h2>
          </div>
        </div>
        {runtimesQuery.isLoading ? (
          <div className="grid gap-4 lg:grid-cols-2">
            {[0, 1].map((item) => (
              <Skeleton key={item} className="h-52 rounded-[var(--radius-panel)]" />
            ))}
          </div>
        ) : runtimesQuery.isError ? (
          <ErrorState error={runtimesQuery.error} onRetry={() => runtimesQuery.refetch()} />
        ) : runtimes.length === 0 ? (
          <EmptyState
            title={t("runtime:controlCenter.emptyTitle")}
            hint={t("runtime:controlCenter.emptyHint")}
          />
        ) : (
          <div className="grid gap-4 lg:grid-cols-2">
            {runtimes.map((runtime) => {
              const employee = employees.find((item) => item.id === runtime.employee_id);
              const image = imageByType.get(runtime.runtime_type);
              return (
                <article
                  key={runtime.id}
                  className="command-panel relative overflow-hidden p-5 transition hover:-translate-y-0.5 hover:border-border-active"
                >
                  <div className="relative flex items-start justify-between gap-4">
                    <div className="flex min-w-0 items-center gap-3">
                      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-primary/25 bg-primary/8 text-primary">
                  ...
                  <dl className="type-caption relative mt-6 grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <dd className="type-telemetry mt-1">
                        ...
                        {image?.update_available ? ` · ${t("runtime:updateAvailable")}` : ""}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-muted-foreground">{t("runtime:card.resources")}</dt>
                      <dd className="type-telemetry mt-1 flex items-center gap-1.5">
                        <Cpu className="h-3 w-3" />
                        {runtime.cpu_limit} / {runtime.memory_limit_mb} MB
                      </dd>
                    </div>
                    <div className="col-span-2">
                      <dt className="text-muted-foreground">{t("runtime:card.container")}</dt>
                      <dd className="type-telemetry mt-1 truncate">
