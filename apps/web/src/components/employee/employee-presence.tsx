...
export function EmployeePresence({
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative flex min-w-0 items-center gap-3 rounded-2xl border border-border bg-background/55 transition duration-200 hover:-translate-y-0.5 hover:bg-surface-elevated",
        status.hoverClass,
        compact ? "p-2.5" : "p-3.5",
      )}
    >
      <span
        className={cn(
          "relative flex shrink-0 items-center justify-center rounded-xl border bg-surface font-semibold",
          status.auraClass,
          status.pulse && "pulse-ring",
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center justify-between gap-2">
          ...
          <span className="type-telemetry text-[9px] text-muted-foreground">LV {level}</span>
        </span>
        ...
        {!compact ? (
          <span className="mt-2 flex items-center justify-between gap-2">
            <span className={cn("type-kicker truncate", status.textClass)}>
              {enumLabel(t, "employee:status", employee.status)}
            </span>
            <span className="truncate font-mono text-[9px] text-muted-foreground">
              {employee.current_task_id
                ? `#EID-${employee.current_task_id}`
                : t("dashboard:workforce.available")}
            </span>
          </span>
        ) : null}
      </span>
