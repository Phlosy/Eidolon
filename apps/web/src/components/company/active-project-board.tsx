          ...
          active.map((project) => {
            ...
            return (
              <Link
                ...
                className="group block rounded-2xl border border-border bg-background/45 p-4 transition hover:border-border-active hover:bg-surface-elevated"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p className="truncate font-semibold">{project.name}</p>
                    <p className="mt-1 truncate text-[11px] text-muted-foreground">
                      {employeeById.get(project.owner_id ?? -1)?.name ??
                        t("dashboard:projects.unassigned")}{" "}
