...
export function CompanyFoundingState({ company }: { company?: Company }) {
  ...
  return (
    <section className="command-panel relative min-h-[620px] overflow-hidden p-6 md:p-10">
      ...
      <div className="relative mx-auto flex max-w-4xl flex-col items-center py-12 text-center md:py-20">
        ...
        <p className="type-kicker mt-8 text-primary">NEW AI COMPANY</p>
        <h1 className="mt-3 max-w-3xl text-3xl font-semibold tracking-[-0.04em] md:text-5xl">
          {company?.name ?? "Eidolon Studio"} 已成立，等待第一位员工
        </h1>
        <p className="mt-5 max-w-2xl text-sm leading-6 text-muted-foreground">
          这是一家刚刚成立、尚未拥有员工的 AI
          公司。通过真实的入职、资源配置和项目交付流程，逐步把它建设成可以工作的软件团队。
        </p>
        <div className="mt-8 grid w-full gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            [Building2, "Company Status", "WAITING FOR FIRST HIRE"],
            [Users, "Employees", "0"],
            [FolderKanban, "Projects", "0"],
            [PackageOpen, "Assets", "0"],
        ...
        <Button className="mt-8 min-w-56" onClick={begin} disabled={start.isPending}>
          {start.isPending ? "正在启动引导…" : "开始建立公司"}
          <ArrowRight className="h-4 w-4" />
        </Button>
        <p className="mt-3 text-[10px] text-muted-foreground">
          引导调用真实 API；刷新可继续，跳过不会创建假数据。
        </p>
      </div>
