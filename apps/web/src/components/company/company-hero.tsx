...
export function CompanyHero({
  ...
  return (
    <Panel className="panel-enter min-h-[340px] border-primary/20 p-6 md:p-8">
      ...
      <div className="relative grid h-full gap-8 xl:grid-cols-[1.1fr_.9fr]">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-[10px] text-muted-foreground">
                {t("dashboard:hero.derived")}
              </span>
            </div>
            <p className="type-kicker mt-7 text-primary">{t("dashboard:hero.commandCenter")}</p>
            <h1 className="type-display mt-3 max-w-3xl uppercase">
          ...
          <div className="mt-8 max-w-xl">
            <div className="mb-2 flex items-center justify-between text-[10px]">
              <span className="font-mono uppercase tracking-[0.12em] text-muted-foreground">
                {t("dashboard:hero.companyExperience")}
              </span>
              <span className="type-telemetry">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-2">
          {metrics.map(([Icon, label, value], index) => (
            <div
              ...
              className={
                index === 0
                  ? "col-span-2 rounded-2xl border border-success/20 bg-success/7 p-5 sm:col-span-1 xl:col-span-2"
                  : "rounded-2xl border border-border bg-background/45 p-4"
              }
            >
              <div className="flex items-center justify-between">
                <Icon className={index === 0 ? "h-4 w-4 text-success" : "h-4 w-4 text-primary"} />
                {index === 0 ? (
              ...
              <p
                className={
                  index === 0
                    ? "type-telemetry mt-7 text-4xl font-semibold"
                    : "type-telemetry mt-5 text-2xl font-semibold"
                }
              >
                {value}
              </p>
          ...
          <div className="rounded-2xl border border-border bg-background/45 p-4">
            ...
            <p className="type-telemetry mt-5 text-2xl font-semibold">{runtimeHealth}%</p>
            <p className="mt-1 text-[10px] text-muted-foreground">
              {t("dashboard:hero.runtimeHealth")}
            </p>
          </div>
        </div>
      </div>
