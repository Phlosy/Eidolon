...
export function ProjectCommandCard({ project, owner }: { project: Project; owner?: Employee }) {
  ...
  const blockers =
    detail?.tasks.filter((task) => task.status === "failed" || task.status === "rejected").length ??
    0;
  const activeMilestone =
    detail?.milestones.find((milestone) => milestone.status === "in_progress") ??
    detail?.milestones.find((milestone) => milestone.status === "pending");
  const health = Math.max(0, 100 - blockers * 30);
  return (
    <Link
      to={`/projects/${project.id}`}
      className="group command-panel relative block overflow-hidden p-5 transition duration-200 hover:-translate-y-1 hover:border-border-active"
    >
      <div className="absolute right-0 top-0 h-32 w-32 bg-[radial-gradient(circle_at_top_right,var(--status-researching-glow),transparent_68%)]" />
      ...
      <div className="relative mt-5">
        ...
        <ProgressTrack value={progress.percent} className="mt-2" />
        <div className="mt-5 grid grid-cols-2 gap-2">
          <Metric
            icon={UserRound}
            label={t("project:command.owner")}
            value={owner?.name ?? t("project:command.unassigned")}
          />
          <Metric
            icon={Flag}
            label={t("project:command.milestone")}
            value={activeMilestone?.name ?? t("project:command.noMilestone")}
          />
          <Metric
            icon={ListChecks}
            label={t("project:command.tasks")}
            value={`${progress.completed}/${progress.total}`}
          />
          <Metric
            icon={Boxes}
            label={t("project:command.artifacts")}
            value={String(detail?.artifacts.length ?? 0)}
          />
        </div>
        <div className="mt-4 flex items-center justify-between rounded-xl border border-border bg-background/45 px-3 py-2.5">
          <span className="flex items-center gap-2 text-[10px] text-muted-foreground">
            ...
            {t("project:command.healthDerived")}
          </span>
          <span
            className={
              blockers > 0
                ? "type-telemetry text-xs font-semibold text-warning"
                : "type-telemetry text-xs font-semibold text-success"
            }
          >
            {health}%
          </span>
